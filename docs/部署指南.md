...
- **文件存储路径**: 修改文件上传和分片存储路径（默认：`./uploads/files` 和 `./uploads/chunks`）

**注意**：应用服务在本地运行，连接Docker中的基础设施服务时使用 `localhost` 即可（Docker已映射端口到宿主机）。
...
- 检查Nacos是否正常运行
- 检查`backend/*/src/main/resources/application.yml`中的Nacos地址配置（默认：`localhost:8848`）
- 检查网络连接
- 确保基础服务（MySQL、Redis、Nacos等）已通过Docker启动
